...
`,
    ...
    obfuscate: function(hash) {
        const keys = KeyDeriver.deriveAllKeys(hash);
        let source = this.demoPayload.trim();

        source = source.replace('EXPIRY_TIME', keys.expiryTimestamp.toString());

        source = IntegrityTrapper.injectIntegrityChecks(source);
        source = TemporalBinder.injectTemporalBinding(source, keys.expiryTimestamp);
        source = ControlFlowFlattener.flatten(source);
        source = SymbolEraser.eraseSymbols(source);

        const encrypted = PayloadEncoder.encryptPayload(source, keys.keyC);

        const payloadTable = PayloadEncoder.formatAsLuaTable(encrypted, keys.segments);
        const decryptionStub = PayloadEncoder.generateDecryptionStub(
            keys.segments,
            keys.expiryTimestamp
        );

        return {
            payloadData: payloadTable,
            decryptionStub: decryptionStub,
            combined: payloadTable + '\n' + decryptionStub,
            expiryTimestamp: keys.expiryTimestamp,
